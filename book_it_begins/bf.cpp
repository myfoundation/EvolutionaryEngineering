...
//------------------------------------------
// УНИВЕРСАЛЬНАЯ МАШИНА ТЬЮРИНГА И ИСПОЛНИТЕЛЬ BRAINFUCK
//------------------------------------------
...
// Математический акт перехода (Чистая функция δ)
void UTM_Cycle(struct UTM* machine)
{
    // Выбор строки в таблице отношений по текущему состоянию и входу
    Step transition = machine->ROM[machine->REG_S][machine->REG_IN];

    // Обновление регистров
    machine->REG_S   = transition.state;  // Переход в новую фазу
    machine->REG_OUT = transition.action; // Формирование команды для исполнителя
}
//------------------------------------------

//------------------------------------------
// ДОМЕННАЯ ОБОЛОЧКА: МАШИНА BRAINFUCK
//------------------------------------------
...
void BF_Physical_Interface(struct UTM* utm, BF_DSM* bfm)
{
    ...
    {
        // 1. Физика -> Математика
        // Преобразуем состояние ячейки в индекс входного алфавита Σ {0, 1}
        utm->REG_IN = (bfm->RAM[bfm->head] == 0) ? 0 : 1;

        // 2. Переход по таблице FSM
        UTM_Cycle(utm);
