...
5. Графический язык в одну функцию 1983 г.  
6. Forth: DSL, VM и RTOS для управления радиотелескопом	  
7. Mame, Cumm  
...
Раздел: Хрестоматийная Машина Тьюринга 1936 г.  
1. Хрестоматийная МТ  
2. Шаг хрестоматийной МТ  
3. Условные переходы  
4. Пример. обходчик FSM таблицы хрестоматийной МТ  
5. Пример. VM «Хрестоматийная МТ»  
...
В 1970-80 гг. установился стандарт оформлять аппаратуру и алгоритмы по образцу доменных машин (автоматов). Из них собирают программы и машины.  
Функция для рисования на экране компьютера `DRAW("цепочка ассемблерных команд для робота-чертильщика")` это доменный язык, встроенный в язык GW-BASIC (Microsoft, 1983 г.).  
«Робот-чертильщик» умеет поднимать и опускать перо, перемещаться по сетке координат, менять толщину и цвет линии, закрашивать замкнутые фигуры, вращать и масштабировать координатные оси. В итоге мы храним не саму картинку, а код её процедурной генерации.  
В играх Another World (1991 г.) от Eric Chahi, Elite (1984 г.) и Frontier Elite II (1993 г.) от David Braben, известного игродела и сооснователя платформы Raspberry Pi, игровой мир рисуется графическими виртуальными машинами с ассемблерами, аналогичными `DRAW DSL`.  
«Каждый объект игрового мира в Frontier Elite II: звезда, планета, корабль, город, космопорт, здание, – это математическая модель с набором вершин, нормалей и списком машинных команд (опкодов) для виртуальной машины.  
При отрисовке объекта, ищется его модель и выполняются опкоды, создающие линии и многоугольники. Получается список отрисовки (raster list / draw list) для каждого кадра игры, с перечнем элементов, для вывода на экран и координатами их размещения».  
  
**Пример. Рисование GW-BASIC, DRAW DSL**  
  
<spoiler title="***">

```

10 SCREEN 12 : CLS ' Включить графический режим и очистить экран

20 ' ЗВЕЗДА
30 DRAW "C14 R60 TA144 R60 TA144 R60 TA144 R60 TA144 R60 BM+20,15 P14,14"

40 SCREEN 12 : CLS ' Включить графический режим и очистить экран

50 ' СНЕЖИНКА
60 DRAW "C13 NU40 TA45 NU40 TA45 NU40 TA45 NU40 TA45 NU40 TA45 NU40 TA45 NU40 TA45 NU40 TA45"
  
```

</spoiler>
  
**Пример. Вызов подпрограмм из DRAW DSL**  
  
<spoiler title="***">

```
10 SCREEN 1 : CLS ' Включить графический режим и очистить экран
20 BOX$ = "U20 R20 D20 L20" ' Команды для рисования квадрата 20х20 единиц

30 ' Встать в первую точку экрана и рисовать первый квадрат
40 DRAW "BM 50,50 X" + VARPTR$(BOX$)

50 ' Встать во вторую точку и рисовать второй квадрат
60 DRAW "BM 100,80 X" + VARPTR$(BOX$)

70 ' Встать в третью точку и рисовать третий квадрат
80 DRAW "BM 180,40 X" + VARPTR$(BOX$)
```

</spoiler>
  
**Команды языка DRAW DSL**  
...
+---------+-------------------------+-----------------------------------+
| TA x    | Поворот координатных    | X' = X*cos(x) - Y*sin(x)          |
|         | осей на x градусов      | Y' = X*sin(x) + Y*cos(x)          |
+---------+-------------------------+-----------------------------------+
| A n     | Поворот на n*90 град.   | Быстрый аналог ТА для углов       |
|         | (Угол = n * 90, n=0..3) | 0,90,180 (зеркало),270 градусов.  |
+---------+-------------------------+-----------------------------------+
...
+---------+-------------------------+-----------------------------------+
| X var$; | Выполнить строку команд | Вызов процедуры на языке DRAW     |
|         | из переменой var BASIC  | (Состояние VM сохраняется)        |
+---------+-------------------------+-----------------------------------+
```
...
Машина Тьюринга (МТ) – не «головка и лента с буквами», а спецификация CPU с памятью произвольного доступа (RAM). В буквальном смысле это инженерно-математический чертёж компьютеров с процессором Intel 8086, Zilog Z80 или виртуальных машин, как Java VM. Объясним его на оболочковой модели.  
...
У PDA память = состояние + стек LIFO. В стек кладут данные наверх и забирают сверху, что позволяет распознавать синтаксис контекстно-свободных языков (Pascal, Algol-60, LISP без макросов).
CPU реализует работу со стеком аппаратно, стековые виртуальные машины – программно. Стеки размещают в памяти `RAM`.  
4. Память с произвольным доступом `RAM (Random Access Memory)` [19] – быстрое хранилище кода и данных, элемент компьютера «архитектуры Фон Неймана». [21] В «Машине Тьюринга» ей соответствует «лента с движущейся головкой». Команды «головке» «перейти влево/вправо» – эквивалент выбора ячейки памяти процессором, команды «считать/записать слово в ячейку» у «Машины Тьюринга» и CPU аналогичны.  
...
Фон-неймановские компьютеры физически (микросхемами) реализуют математическую спецификацию Машины Тьюринга с фиксированным объёмом памяти, то есть `LBA`.  
...
«Ассемблер» – язык для записи машинного кода. Машинный код это последовательность известных процессору `CPU` чисел или «инструкций» в памяти компьютера.  
Регистр `Instruction Pointer` или `IP` (указатель команд) процессора хранит адрес памяти со следующей инструкцией для выполнения. `CPU` читает и распознаёт машинный код (этап `Fetch-Decode`), затем даёт сигналы аппаратуре изменить своё состояние (этап `Execute`).  
Каждая инструкция ассемблера переводит процессор и связанную с ним память из одного состояния (набор значений во всех регистрах, флагах и ячейках ОЗУ) в следующее. Значит, выполнение ассемблерной команды эквивалентно математическому переходу по состояниям конечного автомата.  
Отсюда `FSM-таблица` прямо эквивалентна программе на ассемблере. Строки в ней – заменяют инструкциям ассемблера, а регистр конечного автомата с номером текущего состояния – соответствует регистру `IP`.  
  
  
```
|---------+-----------------------+-----------------------------------+
| КОМПОНЕНТЫ СТРОКИ В ТАБЛИЦЕ FSM | ЭКВИВАЛЕНТ В ЭВМ                  |
|---------+-----------------------+-----------------------------------+
| КОЛОНКА | МАТЕМАТИКА            | ЭЛЕМЕНТ МАШИННОЙ КОМАНДЫ          |
|---------+-----------------------+-----------------------------------+
| S_curr  | Текущее состояние FSM | Значение в регистре IP            |
|         | (S_curr == REG_STATE) | (Смещение в машинном коде)        |
|---------|-----------------------|-----------------------------------|
| In      | Входящий символ       | Аргумент для машинной операции    |
|---------|-----------------------|-----------------------------------|
| S_next  | Следующее состояние   | Инструкция: JMP Новое_Смещение_IP |
|---------|-----------------------|-----------------------------------|
| Out     | Исходящий символ      | Команда аппаратуре                |
|---------|-----------------------|-----------------------------------|
```
  
  
В доменном процессоре мы сами определяем систему команд, количество, размер и назначение именованных ячеек памяти в `CPU` (регистров), модель работы с памятью `RAM`.  
...
В Хрестоматийной Машине Тьюринга 1936 г. нет большой пользы для прикладной практики. Мы используем ее лишь для показа нескольких идей при кодинге автоматов.  
...
Хрестоматийная Машина Тьюринга 1936 г. эквивалентна функции на С `void compute(int *buffer)` обрабатывающий буфер с данными («ленту») по внутреннему алгоритму.  
Состояние МТ в любой момент времени это: { номер текущего состояния `REG_STATE`; номер выбранной для записи/чтения ячейки памяти `REG_HEAD_POS`; массив символов алфавита (чисел) в буфере }. В совокупности эту тройку называют «контекст выполнения» `execution context`.  
Если в функции `compute` сделать дамп этой тройки, получится мгновенный снимок («слепок памяти») вычислительного процесса, из которого его можно восстановить на любой ЭВМ.  
Любые сложные действия в функции `compute` (сложение, умножение, сравнение) собирают комбинацией двух микрокоманд `MVL/MVR` и данных с `FSM_TABLE` и ленты.  
  
  
```
+---------+-----------+-------------------------------------------+
| Мнемо-  | Аргументы | Физическое действие механизма             |
| ника    |           |                                           |
|---------+-----------+-------------------------------------------|
| MVL     | нет       | Физический сдвиг головки влево на 1 ячейку|
|         |           | (Декремент аппаратного указателя адреса). |
|---------+-----------+-------------------------------------------|
| MVR     | нет       | Физический сдвиг головки вправо на 1 яч.  |
|         |           | (Инкремент аппаратного указателя адреса). |
|---------+-----------+-------------------------------------------|
|                        РАСШИРЕНИЕ                               |
|---------+-----------+-------------------------------------------|
| HLT     | нет       | Маркер для остановки машины               |
|         |           | (В хрестоматийной машине его нет).        |
+---------+-----------+-------------------------------------------+
```
  
  
Математически (то есть в теории) это приемлемо, но в инженерной практике не применимо: таблицы будут огромны.  
Даже для тривиальной арифметики в двоичной системе счисления, `{0,1}` сотни строк FSM будут буквально перекладывать «палочки» в ячейках «ленты».  
  
**2. ШАГ ХРЕСТОМАТИЙНОЙ МТ**  
  
Конструктивно МТ, это FSM, перезаписывающий на каждом шаге текущую ячейку на ленте. Чтобы не затереть данные, где это нужно, `I_вх` и `I_вых` в `FSM_TABLE` ставят те же. В итоге лента не меняется: мы пишем в ячейку тот же символ, что с неё прочли.  
...
FSM_TABLE =
+-----+-----+-----+-----+-------+
|S_тек|I_вх | S_но|I_вых| Action|
+-----+-----+-----+-----+-------+
|  A  |  1  |  B  |  2  |  MVR  | <- I_вх != I_вых, ЛЕНТА ИЗМЕНИТСЯ
+-----+-----+-----+-----+-------+
|  B  |  5  |  C  |  5  |  MVR  | <- I_вх == I_вых, ЛЕНТА НЕИЗМЕННА
+-----+-----+-----+-----+-------+
| ... | ... | ... | ... | ...   |
+-----+-----+-----+-----+-------+

// КОНТЕКСТ ХРЕСТОМАТИЙНОЙ МТ

REG_STATE    = A;    // РЕГИСТР С НОМЕРОМ ТЕКУЩЕГО СОСТОЯНИЯ
REG_HEAD_POS = 0;    // РЕГИСТР С НОМЕРОМ ПОЗИЦИИ ГОЛОВКИ
RAM;                 // «ЛЕНТА»

// ШАГ ХРЕСТОМАТИЙНОЙ МТ
IS_RUN = 1
while (IS_RUN)
{

// 1. СЧИТАТЬ СИМВОЛ I_вх С ЛЕНТЫ
    I_in = RAM[REG_HEAD_POS];

// 2. ПО ПАРЕ { S_тек, I_вх} НАЙТИ СООТВЕТСТВУЮЩУЮ СТРОКУ В FSM_TABLE
    STEP = FSM_TABLE[REG_STATE][I_in];
    
// 3. ЗАПИСАТЬ СИМВОЛ I_вых НА ЛЕНТУ
    RAM[REG_HEAD_POS]   = STEP.I_out;

// 4. ИЗМЕНИТЬ НОМЕР СОСТОЯНИЯ НА S_но.
    REG_STATE           = STEP.S_new;
    
// 5. СДВИНУТЬ ПОЗИЦИЮ ГОЛОВКИ СООТВЕТСТВЕННО Action
    switch(STEP.Action)
    {
        case MVL:    REG_HEAD_POS--;    break;
        case MVR:    REG_HEAD_POS++;    break;
...
«Входной» символ с «ленты», управляет выбором строки из FSM таблицы, что эквивалентно условному выбору следующего состояния автомата. На этом построен оператор `if`.  
  

```
КОД НА С
if (data == 0) goto STATE_A;
if (data == 1) goto STATE_B;
if (data == 2) goto STATE_C;

FSM-ТАБЛИЦА АВТОМАТА
+--------+-----+-----+-----+-----+
|S_тек   |I_вх | S_но|I_вых|A_вых|
+--------+-----+-----+-----+-----+
|  S_if  |  0  |  A  |  0  | MVR |
+--------+-----+-----+-----+-----+
|  S_if  |  1  |  B  |  1  | MVR |
...
Функции `compute_add` и `compute_inverse` реализованы хрестоматийными МТ.  
Открыть файл `compute_classic_mt.cpp`, посмотреть его основные блоки, выполнить код пошагово под отладчиком.  
...
Таблица диспетчеризации (Dispatch Table Method) – реализация логического ветвления таблицей с указателями либо множеством блоков кода одинаковой структуры. В компиляторах и виртуальных машинах ими прямо отображают графы переходов (FSM, AST) на линейную память компьютеров и в код программ.  
...
| СХЕМА ВЕТВЛЕНИЯ                                                       |
+-----------------------------------------------------------------------+
| Вход -> [Условие 1?] --Нет--> [Условие 2?] --Нет--> [Условие 3?]      |
|            | Да                  | Да                  | Да           |
|            V                     V                     V              |
...
+-----------------------------------------------------------------------+
| СХЕМА ТАБЛИЦЫ ДИСПЕТЧЕРИЗАЦИИ                                         |
+-----------------------------------------------------------------------+
| Вход (Индекс/Токен) ---> [ ТАБЛИЦА АДРЕСОВ / КОМАНД В RАM ]           |
|                          | Индекс 0 -> Адрес Действия 1               |
|                          | Индекс 1 -> Адрес Действия 2               |
|                          | Индекс 2 -> Адрес Действия 3               |
+-----------------------------------------------------------------------+
```
...
```
+------------------------+------------------------+---------------------+
| Сфера применения       | Объект                 | Цель                |
+------------------------+------------------------+---------------------+
| Системное ПО и ядра ОС | Таблицы векторов       | Быстрый вызов под-  |
|                        | прерываний (IDT)       | программ по номеру  |
|------------------------+------------------------+---------------------|
| Компиляторы            | Таблицы методов класса | Вызов: Имя метода   |
|                        | (vtable)               | -> Адрес в памяти   |
|------------------------+------------------------+---------------------|
| Автоматное кодирование | Таблицы/код переходов  | Реализация автомата |
|                        | по рёбрам графа FSM    |                     |
+------------------------+------------------------+---------------------+
| Интерпретаторы и VM    | Машинный код           | Реализация switch   |
|------------------------+------------------------+---------------------|
```
...
Транслируем таблицы МТ из предыдущего примера в машинный код (блочно-линейную таблицу диспетчеризации с последовательным перебором) и выполним его на доменной VM.  
Открыть файл `compute_classic_mt_vm.cpp`, посмотреть его основные блоки, выполнить код пошагово под отладчиком.  
...
Вычислитель (или «computer») математически эквивалентен одному большому числу, размещённому в ячейках RAM и регистрах CPU. Предназначение каждой команды на BF – менять это число, то есть строить некоторую последовательность чисел, которую можно перенумеровать, начав с нуля. Из-за такого стечения обстоятельств, программа становится эквивалентом переходов по состояниям компьютера.  
В теории автоматов машина останавливается, после перехода в состояние из множества Финальных состояний F. Сама машина (исполнитель команд) не знает финальные состояние в конкретном алгоритме, и не всем алгоритмам нужна остановка. Поэтому в конце байт-кода мы ставим команду `halt`, останавливающую машину.  
...
[30] https://github.com/myfoundation/EvolutionaryEngineering/tree/main/articles_sources/part-01-FSM-Turing-BF  
[31] https://watsonmw.com/fintro/fe2-intro.html  
